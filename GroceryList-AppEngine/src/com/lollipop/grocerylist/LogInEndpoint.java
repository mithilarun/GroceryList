...
import com.lollipop.grocerylist.EMF;
...
@Api(name = "loginendpoint", namespace = @ApiNamespace(ownerDomain = "lollipop.com", ownerName = "lollipop.com", packagePath = "grocerylist"))
public class LogInEndpoint {
	...
	public CollectionResponse<LogIn> listLogIn(
		...
		try {
			...
			Query query = mgr.createQuery("select from LogIn as LogIn");
	...
	private boolean containsLogIn(LogIn login) {
		...
		try {
			LogIn item = mgr.find(LogIn.class, login.getKey());
			if (item == null) {
				contains = false;
			}
		} finally {
